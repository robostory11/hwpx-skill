...
- **작은 파일도 멈춘다** → 환경 문제다. **거의 언제나 보안 모듈 등록이 죽은 경로를
  가리키는 것**이다(2026-09-02 실측). 아래 한 줄로 고친다:
  ```
  taskkill /F /IM Hwp.exe
  python -c "from pyhwpx import Hwp; Hwp.register_regedit()"
  ```
  ★ `RegisterModule` 이 「성공」이라 답해도 **DLL 이 붙었다는 뜻이 아니다** — 레지스트리에
  이름만 있으면 성공이다. 안 붙으면 한글이 승인 창을 띄우는데 `Visible = False` 로 감춘
  뒤라 **아무도 못 누르는 창 앞에서 영영 기다린다.**
  못 고치면 상한을 걸어 건너뛰고 「사람이 한글에서 한 번 열어 저장해 주십시오」를
  **보고에 적는다.** 조용히 넘기지 않는다.
...
### ★★ 내보내기 직전 — 레이아웃과 내용을 **기계가 본다** (2026-09-11 신설)

```bash
python "C:/Users/robo/.claude/skills/hwpx-skill/scripts/layout_guard.py"  문서.hwpx
python "C:/Users/robo/.claude/skills/hwpx-skill/scripts/content_guard.py" 문서.hwpx   # 원고(.md)도 된다
```

**둘은 다른 것을 본다. 하나만 돌리면 나머지 절반이 안 보인다.**

| 도구 | 무엇을 보나 | 못 잡는 것 |
|---|---|---|
| `layout_guard` | **종이에 제대로 찍히는가** — 표 칸이 한 쪽을 넘어 잘렸나, 줄간격이 좁아 글자가 겹치나, 표 머리(`hp:sz`)를 잃었나, 그림이 칸을 넘쳤나. `--paper` 는 한글로 PDF 를 찍어 **문서 안의 글이 종이에 다 있는지** 대조한다 | 글이 제자리에 찍히기만 하면 **내용이 서로 모순돼도 통과** |
| `content_guard` | **문서 안에서 서로 어긋나는 말** — 표는 「완료」인데 같은 행이 「대기」, 「세 가지로 구분」인데 표는 한 행, 「4-1 대조표」인데 4-1 은 다른 절, 상태를 한 가지로 정의해 놓고 다른 절에서 여러 갈래로 나눠 셈, 같은 라벨의 값이 두 곳에서 다름 | 글자가 안 찍히는 것 |

★ **왜 둘 다 필요한가 (2026-09-11 실측).** 타투다 외주용역 결과보고서에서 **같은 날 둘 다 났다** —
  화면 목록 40행 표가 한 쪽을 넘어 **뒤 18행이 말없이 안 찍혔고**(layout),
  이행 상태 칸을 전부 「완료」로 바꾸고 본문 서술을 안 고쳐 **표와 본문이 정면으로 어긋났다**(content).
  뒤엣것은 `validate.py` · `fill_hwpx.py check --strict` · `layout_guard` 를 **전부 통과**했고,
  교차검증 검토자가 읽고서야 나왔다 — *"평가위원이 즉시 허위 기재로 적발할 수 있는 가장 치명적 결함"*(agy).

★ **둘 다 고장을 실제로 주입해** 잡는 것을 확인하고 넣었다 — 정상 문서에서 오탐 0,
  주입본에서 4건 전부 검출. 종료값은 0 통과 / 1 문제 / 2 검사가 깨짐이다.

★ `content_guard` 는 **원고(.md)에 돌리는 것이 낫다** — hwpx 는 생성 결과라,
  원고에서 잡으면 다시 생성할 필요가 없다.
