...
| 2. 들어 있는 것 | 그림·도장이 빠졌나 · 이름이 같아도 알맹이가 바뀌었나(CRC) · **쪽 설정** |
...
| 하이퍼링크 주소 | 겉글자만 본다 |
| 같은 글자 문단이 여럿일 때 **어느 문단**의 서식이 바뀌었는지 | 글자로 짝짓기 때문에 하나가 지워지면 짝이 밀려 엉뚱한 문단을 짚을 수 있다. **바뀌었다는 사실은 놓치지 않지만 위치는 참고로만 볼 것** |
